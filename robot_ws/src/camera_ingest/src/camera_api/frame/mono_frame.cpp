...
class RGBAFrame: public nssc::framestruct::MonoFrame
{
    public:
        ...
        {
            if (CUDA_FAILED(cudaConvertColor(rgbBuf->dImageBuf, IMAGE_RGB8, this->rgba_buf.dImageBuf, IMAGE_RGBA8, this->node->g_config.frame_config.mono_x_res, this->node->g_config.frame_config.mono_y_res)))
            {
...
class I420Frame: public nssc::framestruct::MonoFrame
{
    public:
        void convert(nssc::framestruct::FrameBuffer* rgbBuf) override
        {
            if (CUDA_FAILED(cudaConvertColor(rgbBuf->dImageBuf, IMAGE_RGB8, this->rgba_buf.dImageBuf, IMAGE_I420, this->node->g_config.frame_config.mono_x_res, this->node->g_config.frame_config.mono_y_res)))
            {
...
class UYVYFrame: public nssc::framestruct::MonoFrame
{
    public:
        void convert(nssc::framestruct::FrameBuffer* rgbBuf) override
        {
            ...
            int ret = nppiRGBToYUV422_8u_C3C2R((uint8_t*)rgbBuf->dImageBuf, this->node->g_config.frame_config.mono_x_res * 3, (uint8_t*)rgba_buf.dImageBuf, this->node->g_config.frame_config.mono_x_res * 2, oSizeROI);
            if(ret != 0)
            {
